...
private let settingsLogger = PrismaVoiceLog.settings
...
struct SettingsFeature {
  ...
  private func applyCapturedHotKey(key: Key?, modifiers: Modifiers, for target: HotKeyCaptureTarget, state: inout State) {
    switch target {
    case .recording:
      state.$prismaVoiceSettings.withLock {
        $0.hotkey.key = key
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case let .setLiveTranscriptionEnabled(enabled):
        state.$prismaVoiceSettings.withLock { $0.liveTranscriptionEnabled = enabled }
        return .none
      ...
      case .clearPasteLastTranscriptHotkey:
        state.$prismaVoiceSettings.withLock { $0.pasteLastTranscriptHotkey = nil }
        return .none
      ...
      case let .toggleOpenOnLogin(enabled):
        state.$prismaVoiceSettings.withLock { $0.openOnLogin = enabled }
        return .run { _ in
          if enabled {
      ...
      case let .toggleShowDockIcon(enabled):
        state.$prismaVoiceSettings.withLock { $0.showDockIcon = enabled }
        return .run { _ in
          await MainActor.run {
      ...
      case let .togglePreventSystemSleep(enabled):
        state.$prismaVoiceSettings.withLock { $0.preventSystemSleep = enabled }
        return .none

      case let .setUseClipboardPaste(enabled):
        state.$prismaVoiceSettings.withLock { $0.useClipboardPaste = enabled }
        return .none

      case let .setCopyToClipboard(enabled):
        state.$prismaVoiceSettings.withLock { $0.copyToClipboard = enabled }
        return .none

      case let .setRecordingAudioBehavior(behavior):
        state.$prismaVoiceSettings.withLock { $0.recordingAudioBehavior = behavior }
        return .none

      case let .toggleSuperFastMode(enabled):
        state.$prismaVoiceSettings.withLock { $0.superFastModeEnabled = enabled }
        return .run { _ in
      ...
      case let .setMinimumKeyTime(value):
        state.$prismaVoiceSettings.withLock { $0.minimumKeyTime = value }
        return .none

      case let .setOutputLanguage(language):
        state.$prismaVoiceSettings.withLock { $0.outputLanguage = language }
        return .none

      case let .setSelectedMicrophoneID(deviceID):
        state.$prismaVoiceSettings.withLock { $0.selectedMicrophoneID = deviceID }
        return .none

      case let .setSoundEffectsEnabled(enabled):
        state.$prismaVoiceSettings.withLock { $0.soundEffectsEnabled = enabled }
        return .none

      case let .setSoundEffectsVolume(volume):
        state.$prismaVoiceSettings.withLock { $0.soundEffectsVolume = volume }
        return .none
      ...
      case let .modelDownload(.selectModel(newModel)):
        // Also store it in prismaVoiceSettings:
        state.$prismaVoiceSettings.withLock {
      ...
      case let .toggleSaveTranscriptionHistory(enabled):
        state.$prismaVoiceSettings.withLock { $0.saveTranscriptionHistory = enabled }
      ...
      case let .setMaxHistoryEntries(maxHistoryEntries):
        state.$prismaVoiceSettings.withLock { $0.maxHistoryEntries = maxHistoryEntries }
        return .none

      case let .setModifierSide(kind, side):
        guard state.prismaVoiceSettings.hotkey.key == nil else { return .none }
        state.$prismaVoiceSettings.withLock {
      ...
      case let .setWordRemovalsEnabled(enabled):
        state.$prismaVoiceSettings.withLock { $0.wordRemovalsEnabled = enabled }
        return .none
