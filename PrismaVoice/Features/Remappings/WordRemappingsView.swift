...
struct WordRemappingsView: View {
		...
		GroupBox {
			VStack(alignment: .leading, spacing: 10) {
				Toggle(
					...
					isOn: Binding(
						get: { store.prismaVoiceSettings.wordRemovalsEnabled },
				...
				LazyVStack(alignment: .leading, spacing: 6) {
					ForEach(store.prismaVoiceSettings.wordRemovals) { removal in
	...
	private var remappingsSection: some View {
		GroupBox {
			VStack(alignment: .leading, spacing: 10) {
				...
				LazyVStack(alignment: .leading, spacing: 6) {
					ForEach(store.prismaVoiceSettings.wordRemappings) { remapping in
	...
	private func removalBinding(for id: UUID) -> Binding<WordRemoval>? {
		guard let index = store.prismaVoiceSettings.wordRemovals.firstIndex(where: { $0.id == id }) else {
			return nil
		}
		return Binding(
			get: { store.prismaVoiceSettings.wordRemovals[index] },
			set: { store.send(.updateWordRemoval($0)) }
		)
	}

	private func remappingBinding(for id: UUID) -> Binding<WordRemapping>? {
		guard let index = store.prismaVoiceSettings.wordRemappings.firstIndex(where: { $0.id == id }) else {
			return nil
		}
		return Binding(
			get: { store.prismaVoiceSettings.wordRemappings[index] },
	...
	private var previewText: String {
		...
		if store.prismaVoiceSettings.wordRemovalsEnabled {
			output = WordRemovalApplier.apply(output, removals: store.prismaVoiceSettings.wordRemovals)
		}
		output = WordRemappingApplier.apply(output, remappings: store.prismaVoiceSettings.wordRemappings)
		return output
	}
}
