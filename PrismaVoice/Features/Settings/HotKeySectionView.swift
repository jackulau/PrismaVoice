...
struct HotKeySectionView: View {
    ...
    var body: some View {
        Section("Hot Key") {
            let hotKey = store.prismaVoiceSettings.hotkey
            ...
            Label {
                Toggle(
                    ...
                    isOn: Binding(
                        get: { store.prismaVoiceSettings.doubleTapLockEnabled },
            ...
            Label {
                Picker("Recording mode", selection: $store.prismaVoiceSettings.recordingMode) {
            ...
            if store.prismaVoiceSettings.hotkey.key == nil {
                Label {
                    Slider(
                        value: Binding(
                            get: { store.prismaVoiceSettings.minimumKeyTime },
                            set: { store.send(.setMinimumKeyTime($0)) }
                        ),
                        in: 0.0 ... 2.0,
                        step: 0.1
                    ) {
                        Text("Ignore below \(store.prismaVoiceSettings.minimumKeyTime, specifier: "%.1f")s")
                    }
                } icon: {
