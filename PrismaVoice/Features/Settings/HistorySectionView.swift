		...
		Section {
			Label {
				Toggle("Save Transcription History", isOn: Binding(
					get: { store.prismaVoiceSettings.saveTranscriptionHistory },
			...
			if store.prismaVoiceSettings.saveTranscriptionHistory {
				Label {
					HStack {
						Text("Maximum History Entries")
						Spacer()
						Picker("", selection: Binding(
							get: { store.prismaVoiceSettings.maxHistoryEntries ?? 0 },
							set: { newValue in
				...
				if store.prismaVoiceSettings.maxHistoryEntries != nil {
...
private struct PasteLastTranscriptHotkeyRow: View {
	...
	var body: some View {
		let pasteHotkey = store.prismaVoiceSettings.pasteLastTranscriptHotkey
