...
private let appLogger = PrismaVoiceLog.app
private let cacheLogger = PrismaVoiceLog.caches

class PrismaVoiceAppDelegate: NSObject, NSApplicationDelegate {
	...
	private var shouldOpenForegroundUIOnLaunch: Bool {
		!(launchedAtLogin && !prismaVoiceSettings.showDockIcon)
	}
	...
	private func startLifecycleTasksIfNeeded() {
		Task { @MainActor in
			await PrismaVoiceApp.appStore.send(.task).finish()
		}
	}
	...
	func presentMainView() {
		...
		let transcriptionStore = PrismaVoiceApp.appStore.scope(state: \.transcription, action: \.transcription)
	...
	func presentSettingsView() {
		...
		let settingsView = AppView(store: PrismaVoiceApp.appStore)
	...
	private func updateAppMode() {
		appLogger.debug("showDockIcon = \(self.prismaVoiceSettings.showDockIcon)")
		if self.prismaVoiceSettings.showDockIcon {
