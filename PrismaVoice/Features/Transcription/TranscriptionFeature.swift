...
private let transcriptionFeatureLogger = PrismaVoiceLog.transcription
  ...
  func startHotKeyMonitoringEffect() -> Effect<Action> {
    .run { send in
      ...
      let token = keyEventMonitor.handleInputEvent { inputEvent in
        ...
        hotKeyProcessor.hotkey = prismaVoiceSettings.hotkey
        hotKeyProcessor.recordingMode = prismaVoiceSettings.recordingMode
        hotKeyProcessor.doubleTapLockEnabled = prismaVoiceSettings.doubleTapLockEnabled
        hotKeyProcessor.minimumKeyTime = prismaVoiceSettings.minimumKeyTime
...
private extension TranscriptionFeature {
  func handleStartRecording(_ state: inout State) -> Effect<Action> {
    ...
    return .merge(
      ...
      .run { [sleepManagement, preventSleep = state.prismaVoiceSettings.preventSystemSleep] _ in
        // Play sound immediately for instant feedback
        soundEffect.play(.startRecording)

        if preventSleep {
          await sleepManagement.preventSleep(reason: "PrismaVoice Recording")
        }
  ...
  func handleStopRecording(_ state: inout State) -> Effect<Action> {
    ...
    let decision = RecordingDecisionEngine.decide(
      .init(
        hotkey: state.prismaVoiceSettings.hotkey,
        minimumKeyTime: state.prismaVoiceSettings.minimumKeyTime,
    ...
    let minimumKeyTime = state.prismaVoiceSettings.minimumKeyTime
    let hotkeyHasKey = state.prismaVoiceSettings.hotkey.key != nil
    ...
    state.error = nil
    let model = state.prismaVoiceSettings.selectedModel
    let language = state.prismaVoiceSettings.outputLanguage
...
private extension TranscriptionFeature {
  func handleTranscriptionResult(
    ...
    let remappings = state.prismaVoiceSettings.wordRemappings
    let removalsEnabled = state.prismaVoiceSettings.wordRemovalsEnabled
    let removals = state.prismaVoiceSettings.wordRemovals
  ...
  func finalizeRecordingAndStoreTranscript(
    ...
    if prismaVoiceSettings.saveTranscriptionHistory {
      ...
      transcriptionHistory.withLock { history in
        ...
        if let maxEntries = prismaVoiceSettings.maxHistoryEntries, maxEntries > 0 {
...
private enum ForceQuitCommandDetector {
  static func matches(_ text: String) -> Bool {
    ...
    return normalized == "force quit prismavoice now" || normalized == "force quit prismavoice"
  }
