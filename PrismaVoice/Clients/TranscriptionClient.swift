...
private let transcriptionLogger = PrismaVoiceLog.transcription
private let modelsLogger = PrismaVoiceLog.models
private let parakeetLogger = PrismaVoiceLog.parakeet
...
actor TranscriptionClientLive {
  ...
  func deleteModel(variant: String) async throws {
    ...
    if isQwen(variant) {
      try await qwen.deleteModel(modelName: variant)
      if currentModelName == variant { unloadCurrentModel() }
      return
    }
  ...
  func transcribe(
    ...
    if isQwen(model) {
      transcriptionLogger.notice("Transcribing with Qwen3-ASR model=\(model) file=\(url.lastPathComponent)")
      let startLoad = Date()
