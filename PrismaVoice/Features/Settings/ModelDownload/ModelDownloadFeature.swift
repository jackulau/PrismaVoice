...
public struct ModelDownloadFeature {
	...
	private func reduce(state: inout State, action: Action) -> Effect<Action> {
		...
		case let .selectModel(model):
			...
			state.$prismaVoiceSettings.withLock { $0.selectedModel = resolved }
		...
		case let .modelsLoaded(recommended, available):
			...
			if state.prismaVoiceSettings.selectedModel.contains("*") || state.prismaVoiceSettings.selectedModel.contains("?") {
				if let resolved = resolvePattern(state.prismaVoiceSettings.selectedModel, from: available) {
					state.$prismaVoiceSettings.withLock { $0.selectedModel = resolved }
				}
			}
			...
			updateBootstrapState(&state)
			if !state.anyModelDownloaded && !state.prismaVoiceSettings.hasCompletedModelBootstrap {
				let preferred = state.recommendedModel.isEmpty ? state.prismaVoiceSettings.selectedModel : state.recommendedModel
				if !preferred.isEmpty {
					state.$prismaVoiceSettings.withLock { $0.selectedModel = preferred }
					updateBootstrapState(&state)
				}
			}
			return .none
		...
		case .downloadSelectedModel:
			guard !state.prismaVoiceSettings.selectedModel.isEmpty else { return .none }
			state.downloadError = nil
			state.isDownloading = true
			let selected = state.prismaVoiceSettings.selectedModel
		...
		case let .downloadProgress(progress):
			...
			if state.downloadingModelName == state.prismaVoiceSettings.selectedModel {
		...
		case let .downloadCompleted(result):
			...
			case let .success(name):
				...
				state.$prismaVoiceSettings.withLock { settings in
...
extension ModelDownloadFeature.State {
	...
	private var prefersEnglishParakeet: Bool {
		guard let language = prismaVoiceSettings.outputLanguage?.lowercased(), !language.isEmpty else {
			return false
		}
