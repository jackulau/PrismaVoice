...
private let recordingLogger = PrismaVoiceLog.recording
private let mediaLogger = PrismaVoiceLog.media
...
actor RecordingClientLive {
  ...
  private func handleCaptureEnvironmentChange(reason: String, forceRestart: Bool) async {
    ...
    if prismaVoiceSettings.superFastModeEnabled {
  ...
  private func resolvePreferredInputDevice() -> AudioDeviceID? {
    if let selectedDeviceIDString = prismaVoiceSettings.selectedMicrophoneID,
  ...
  private func currentCaptureMode() -> CaptureRecordingMode {
    prismaVoiceSettings.superFastModeEnabled ? .superFast : .standard
  }
  ...
  private func ensureInputDeviceUnmuted() {
    ...
    if let selectedIDString = prismaVoiceSettings.selectedMicrophoneID,
  ...
  func startRecording() async {
    ...
    switch prismaVoiceSettings.recordingAudioBehavior {
    case .pauseMedia:
