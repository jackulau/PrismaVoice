...
struct AppFeature {
  ...
  var body: some ReducerOf<Self> {
    ...
    Reduce { state, action in
      ...
      case .transcription(.modelMissing):
        PrismaVoiceLog.app.notice("Model missing - activating app and switching to settings")
        state.activeTab = .settings
        state.settings.shouldFlashModelSection = true
        return .run { send in
          await MainActor.run {
            PrismaVoiceLog.app.notice("Activating app for model missing")
  ...
  private func startPasteLastTranscriptMonitoring() -> Effect<Action> {
    .run { send in
      ...
      let token = keyEventMonitor.handleKeyEvent { keyEvent in
        ...
        guard let pasteHotkey = prismaVoiceSettings.pasteLastTranscriptHotkey,
  ...
  private func ensureSelectedModelReadiness() -> Effect<Action> {
    .run { send in
      @Shared(.prismaVoiceSettings) var prismaVoiceSettings: PrismaVoiceSettings
      @Shared(.modelBootstrapState) var modelBootstrapState: ModelBootstrapState
      let selectedModel = prismaVoiceSettings.selectedModel
