...
struct MicrophoneSelectionSectionView: View {
	...
	private var selectedDeviceTag: Binding<String> {
		Binding(
			get: { store.prismaVoiceSettings.selectedMicrophoneID ?? Self.systemDefaultTag },
			set: { newValue in
	...
	private var missingSelectedDeviceID: String? {
		guard let selectedID = store.prismaVoiceSettings.selectedMicrophoneID,
	...
	private var pickerIdentity: String {
		[
			...
			store.prismaVoiceSettings.selectedMicrophoneID ?? Self.systemDefaultTag,
